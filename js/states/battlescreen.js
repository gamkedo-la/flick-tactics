...
var battlescreenCharacterDisplaySize = 90;
...
var activeMUnitOffset = vec2();
var passiveMUnitOffset = vec2();
...
function battlescreenSetup() {
    ...
    if(isMobile()) {
        battlescreenCharacterDisplaySize = 150;
        battlescreenHealthBarSize = vec2(240, 80);
        battlescreenSingleBarSize = vec2(20, 70);
        battlescreenGapBetweenBar = 4;
        activeMUnitOffset = vec2(-96 * pixelSize, 96 * pixelSize);
        passiveMUnitOffset = vec2(96 * pixelSize, (gameHeight / 2) - (96 * pixelSize));
    } else {
        battlescreenCharacterDisplaySize = 90;
        battlescreenHealthBarSize = vec2(204, 60);
        battlescreenSingleBarSize = vec2(18, 56);
        battlescreenGapBetweenBar = 4;
        activeMUnitOffset = vec2();
        passiveMUnitOffset = vec2();
    }
}
...
function battlescreenDraw(deltaTime) {
    ...
    drawRect(spritesRenderer, vec2(0, 0),
        vec2(battlescreenCharacterDisplaySize * pixelSize, battlescreenCharacterDisplaySize * pixelSize), true, "#006600");
    drawRect(spritesRenderer, vec2(gameWidth - (battlescreenCharacterDisplaySize * pixelSize), gameHeight - (battlescreenCharacterDisplaySize * pixelSize)),
        vec2(battlescreenCharacterDisplaySize * pixelSize, battlescreenCharacterDisplaySize * pixelSize), true, "#660000");
    ...
    for (let i = 0; i < 3; i++) {
        drawSheet(map.getTileTypeFromPosition(activeMUnit.mapPosition), vec2(200 * pixelSize, (150 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(activeMUnitOffset), tileSc);
    }
    for (let i = 0; i < 2; i++) {
        drawSheet(map.getTileTypeFromPosition(activeMUnit.mapPosition), vec2(300 * pixelSize, (200 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(activeMUnitOffset), tileSc);
    }
    ...
    for (let i = 0; i < 3; i++, activeTHp -= 2) {
        if (activeTHp > 0)
        {
            activeMUnit.unit.draw(activeTeamID, vec2((200 * pixelSize) - recoilPositionOffset, (150 * pixelSize) + (100 * pixelSize * i)).add(activeMUnitOffset),
            ...
            if(activeMUnit.unit.type == RIFLE_MECH) {
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime && gameTime % 160 < 80) {
                    drawSheet(rifleMuzzles[muzzleIndex], activeMUnit.unit.position.add(vec2((200 * pixelSize) + muzzleSpace, (150 * pixelSize) + (100 * pixelSize * i))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                ...
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime) {
                    ...
                    drawSheet(cannonMuzzles[Math.floor(muzzleIndex)], activeMUnit.unit.position.add(vec2((200 * pixelSize) + muzzleSpace, (150 * pixelSize) + (100 * pixelSize * i))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                    if(!battlescreenAttackSound) {
                ...
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime) {
                    ...
                    if(muzzleIndex > 1 && muzzleIndex < 2)
                        ...
                        recoilPositionOffset = lerp(recoilPositionOffset, 0.0, 0.25);
                    drawSheet(artilleryMuzzles[Math.floor(muzzleIndex)], activeMUnit.unit.position.add(vec2((200 * pixelSize) + (muzzleSpace/1.5), (150 * pixelSize) + (100 * pixelSize * i) - (muzzleSpace/1.5))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                    if(!battlescreenAttackSound) {
    ...
    for (let i = 0; i < 2; i++, activeTHp -= 2) {
        if (activeTHp > 0)
        {
            activeMUnit.unit.draw(activeTeamID, vec2((300 * pixelSize) - recoilPositionOffset, (200 * pixelSize) + (100 * pixelSize * i)).add(activeMUnitOffset),
                vec2(pixelSize, pixelSize));

            if(activeMUnit.unit.type == RIFLE_MECH) {
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime && gameTime % 160 < 80) {
                    drawSheet(rifleMuzzles[muzzleIndex], activeMUnit.unit.position.add(vec2((300 * pixelSize) + muzzleSpace + recoilPositionOffset, (200 * pixelSize) + (100 * pixelSize * i))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                    recoilPositionOffset = lerp(recoilPositionOffset, 0.0, 0.75);
                } else {
                    muzzleIndex = Math.floor(Math.random() * 3);
                ...
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime) {
                    ...
                    if(muzzleIndex > 1 && muzzleIndex < 2)
                        ...
                        recoilPositionOffset = lerp(recoilPositionOffset, 0.0, 0.25);
                    drawSheet(cannonMuzzles[Math.floor(muzzleIndex)], activeMUnit.unit.position.add(vec2((300 * pixelSize) + muzzleSpace, (200 * pixelSize) + (100 * pixelSize * i))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                }
            } else if (activeMUnit.unit.type == ARTILLERY_MECH) {
                if(battlescreenTimer < battlescreenDelay/1.25 && battlescreenTimer > battlescreenSwitcherHighTime) {
                    ...
                    if(muzzleIndex > 1 && muzzleIndex < 2)
                        ...
                        recoilPositionOffset = lerp(recoilPositionOffset, 0.0, 0.25);
                    drawSheet(artilleryMuzzles[Math.floor(muzzleIndex)], activeMUnit.unit.position.add(vec2((300 * pixelSize) + (muzzleSpace/1.5), (200 * pixelSize) + (100 * pixelSize * i) - (muzzleSpace/1.5))).add(activeMUnitOffset),
                        vec2(pixelSize, pixelSize));
                }
            }
        }
    ...
    for (let i = 0; i < 3; i++) {
        drawSheet(map.getTileTypeFromPosition(passiveMUnit.mapPosition), vec2(gameWidth - (200 * pixelSize), (150 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(passiveMUnitOffset),
        ...
        if((prevHp >= 4 && passiveMUnit.hp < 4 && i == 2) || (prevHp >= 2 && passiveMUnit.hp < 2 && i == 1) || (prevHp > 0 && passiveMUnit.hp <= 0 && i == 0)) {
            destroyParticles.push(new TileParticle(vec2(gameWidth - (200 * pixelSize), (150 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(passiveMUnitOffset), unitDestroySequence, null, false));
    ...
    for (let i = 0; i < 2; i++) {
        drawSheet(map.getTileTypeFromPosition(passiveMUnit.mapPosition), vec2(gameWidth - (300 * pixelSize), (200 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(passiveMUnitOffset),
            vec2(battlescreenTileSize * pixelSize, battlescreenTileSize * pixelSize));

        if((prevHp >= 8 && passiveMUnit.hp < 8 && i == 1) || (prevHp >= 6 && passiveMUnit.hp < 6 && i == 0)) {
            destroyParticles.push(new TileParticle(vec2(gameWidth - (300 * pixelSize), (200 * pixelSize) + (100 * pixelSize * i) + (battlescreenTileOffset * pixelSize)).add(passiveMUnitOffset), unitDestroySequence, null, false));
            if(!battlescreenDamageSound) {
    ...
    for (let i = 0; i < 3; i++, passiveTHp -= 2) {
        if (passiveTHp > 0)
            passiveMUnit.unit.draw(passiveTeamID, vec2(gameWidth - (200 * pixelSize), (150 * pixelSize) + (100 * pixelSize * i)).add(passiveMUnitOffset),
                vec2(pixelSize, pixelSize), true);
    }
    for (let i = 0; i < 2; i++, passiveTHp -= 2) {
        if (passiveTHp > 0)
            passiveMUnit.unit.draw(passiveTeamID, vec2(gameWidth - (300 * pixelSize), (200 * pixelSize) + (100 * pixelSize * i)).add(passiveMUnitOffset),
                vec2(pixelSize, pixelSize), true);
    }
