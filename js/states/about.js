...
var creditsListMob = [
    "Developed by Members of HomeTeam GameDev (Outpost)",
    "",
    "Bilal A. Cheema:  Project lead, core gameplay, building",
    "and tile art, logo, palette, battle screen, mechs art,",
    "zoom functionality, pathfinding, map art, audio integration,",
    "dialog system, map editor, power meter, enemy AI, particles,",
    "save and load, Zareem and Guru body, Taja faces, assorted,",
    "improvements and fixes",
    "",
    "Kyle Knutson:  Concept art, body and faces for Hulu, Jonah,",
    "and generic soldier, additional Guru and Zareem faces,",
    "level design (Great Divide)",
    "",
    "Ryan Young:  Level design (maps 4, 5, 6, 7, two faction)",
    "",
    "Roc Lee:  World music, main menu music, UI sounds, action sounds",
    "",
    "Ashleigh M.:  Zareem concept art and mock ups, stats screen flow,",
    "building and player count",
    "",
    "Justin Chin:  Level design (tutorial map 1, map 2, map 3), writing",
    "(level 1 tutorial, character power, win, lose)",
    "",
    "Alan Zaring:  Music loop for player CO",
    "",
    "Gonzalo Delgado:  Rifle mech idle animation, case-sensitive fix",
    ...
    "Rutger McKenna:  Soldier sprites with walk and fly animations",
    "",
    "Allan Regush:  Help menu, button event support, skip to",
    "gameplay debug feature",
    "",
    "Jonathan Peterson:  Audio on/off mute toggle buttons,",
    "additional case-sensitive fix",
    "",
    "H Trayford:  Question mark animation, improved",
...
function aboutSetup() {
    ...
    var fontStr = ((isMobile() ? 20 : 12) * pixelSize).toString() + "px " + uiContext.fontFamily;

    labelGroup = [];
    if(isMobile())
        for(var i = 0; i < creditsListMob.length; i++)
            labelGroup.push(new Label(tr(), creditsListMob[i], fontStr, i == 0 ? "cyan" : "white"));
    else
        for(var i = 0; i < creditsList.length; i++)
            labelGroup.push(new Label(tr(), creditsList[i], fontStr, i == 0 ? "cyan" : "white"));

    about.push(new FlexGroup(tr(vec2(0, 20 * pixelSize), vec2(gameWidth, isMobile() ? (gameHeight - (120 * pixelSize)) : (gameHeight / 2))),
        new SubState(tr(), labelGroup), false, vec2(), vec2(1, isMobile() ? creditsListMob.length : creditsList.length), true));

    aboutToStartButton = new TextButton(
        isMobile() ? tr(vec2(0, gameHeight - (80 * pixelSize)), vec2(gameWidth, 80 * pixelSize))
